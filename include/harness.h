...
int algoKernelArgs(int argc, char** argv,
        ...
        uint64_t& maxIters, uint32_t& numParts, string& setting, bool& noPreprocess, bool& isCluster, bool& isRotationBased, bool& undirected,
    ...
    while ((ch = getopt(argc, argv, "t:g:i:m:p:s:n:c:r:uh")) != -1) {
        switch (ch) {
            ...
            case 'c':
                ...
                break;
            case 'r':
                std::stringstream(optarg) >> isRotationBasedFlag;
                if (isRotationBasedFlag == 1)
