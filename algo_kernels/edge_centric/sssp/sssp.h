...
struct SSSPUpdate {
    ...
    static Task genTask(const SSSPUpdate& update1, const SSSPUpdate& update2, uint64_t vertexIndex, uint64_t srcTid, uint64_t dstTid, bool isFinal=false) {
        ...
        struct Task task = {MIN_UINT_WITH_PARENT, vertexIndex, false, (void*)mwp, (uint64_t)-1, dstTid, srcTid, 0, isFinal};
        return task;
    }
...
protected:
    ...
    Task genDecryptDistanceAndPredecessorTask(const SSSPData<EdgeWeightType>& data, uint64_t vertexIndex) const {
        DecryptEntryPair<uint32_t, uint64_t>* decp = new DecryptEntryPair<uint32_t, uint64_t>();
        ...
        decp->operand.enc = data.enc;
        decp->isOperandEncrypted[0] = true;
        decp->isOperandEncrypted[1] = true;

        struct Task task = {DEC_PAIR_UINT_ULONG, vertexIndex, false, (void*)decp, (uint64_t)-1, this->getVertexTid(vertexIndex), this->getVertexTid(vertexIndex)};
        return task;
    }

    void writeDecryptDistanceAndPredecessorTaskResult(const Task& task, Ptr<VertexType>& dst) const {
        if (!task.finished) {
            throw ResultNotReadyException("Task result not ready yet!\n");
        }
        DecryptEntryPair<uint32_t, uint64_t>* decp = (DecryptEntryPair<uint32_t, uint64_t>*)task.buf;
        auto& data = dst->data();

        if (decp->isOperandEncrypted[0]) {
            // Exception
            throw InvalidArgumentException("Expect plain value!\n");
        }

        data.isEncrypted = false;
        data.distance = decp->operand.plain_a;
        data.predecessor = decp->operand.plain_b;
    ...
    void onAlgoKernelEnd(Ptr<GraphTileType>& graph) const {
        ...
        // Distance and Predecessor decryption task generation
        for (auto vertexIter = graph->vertexIter(); vertexIter != graph->vertexIterEnd(); ++vertexIter) {
            auto& v = vertexIter->second;
            if (v->data().isEncrypted) {
                taskq.push(genDecryptDistanceAndPredecessorTask(v->data(), v->vid()));
            }
        }
        ...
        while (!taskq.empty()) {
            ...
            writeDecryptDistanceAndPredecessorTaskResult(ret, v);
