#ifndef ALGO_KERNELS_EDGE_CENTRIC_BFS_BFS_H_
#define ALGO_KERNELS_EDGE_CENTRIC_BFS_BFS_H_
...
//    };
    ...
    BFSData(const GraphGASLite::VertexIdx&)
...
struct BFSUpdate {
    ...
    BFSUpdate(const EdgeWeightType distance_ = INF(EdgeWeightType), const GraphGASLite::VertexIdx& predecessor_ = INV_VID)
        : distance(distance_), predecessor(predecessor_), isEncrypted(false)
    {
        // Nothing else to do.
    }

    BFSUpdate(const CipherEntry& c)
        : enc(c), isEncrypted(true)
    {
        // Nothing else to do.
    }

    BFSUpdate& operator+=(const BFSUpdate& update) {
        // Min of distance.
    ...
    static Task genTask(const BFSUpdate& update1, const BFSUpdate& update2, uint64_t vertexIndex, uint64_t srcTid, uint64_t dstTid, bool isFinal=false, bool isDummy=false) {
    ...
    // static Task genEmptyTask(const BFSUpdate& update1, const BFSUpdate& update2, uint64_t vertexIndex, uint64_t srcTid, uint64_t dstTid, bool isFinal=false) {
    //     MinWithParent<uint32_t, uint64_t>* mwp = new MinWithParent<uint32_t, uint64_t>();
    //     struct Task task = {MIN_UINT_WITH_PARENT, -1, false, (void*)mwp, -1, -1, -1, 0, isFinal};
    //     return task;
    // }

    static BFSUpdate<EdgeWeightType> getTaskResult(const struct Task& task) {
        if (!task.finished) {
        ...
        if (!mwp->isOperandEncrypted[0]) {
            BFSUpdate<EdgeWeightType> result = BFSUpdate<EdgeWeightType>(mwp->operands[0].plain_val, GraphGASLite::VertexIdx(mwp->operands[0].plain_parent));
            return result;
        } else {
            BFSUpdate<EdgeWeightType> result = BFSUpdate<EdgeWeightType>(mwp->operands[0].enc);
    ...
    static BFSUpdate<EdgeWeightType> splitRandomShareFromUpdate(const uint32_t srcTid, const uint32_t dstTid, BFSUpdate<EdgeWeightType>& update) {
        // The update variable must be encrypted by the public key of dstTid
        BFSUpdate<EdgeWeightType> update_share;
    ...
    static void mergeUpdateShare(const uint32_t dstTid, BFSUpdate<EdgeWeightType>& update, BFSUpdate<EdgeWeightType>& update_share) {
        CryptoUtil& cryptoUtil = CryptoUtil::getInstance();
...
void print(const BFSUpdate<uint32_t>& update) {
    // printf("BFSUpdate---------\n");
    // printf("count %d\n", update.count.cnt());
...
class BFSEdgeCentricAlgoKernel : public GraphGASLite::SSEdgeCentricAlgoKernel<GraphTileType> {
public:
    static Ptr<BFSEdgeCentricAlgoKernel> instanceNew(const string& name,
            const GraphGASLite::VertexIdx& src) {
        return Ptr<BFSEdgeCentricAlgoKernel>(new BFSEdgeCentricAlgoKernel(name, src));
    }
...
protected:
    ...
    std::pair<UpdateType, bool> scatter(const GraphGASLite::IterCount& iter, Ptr<VertexType>& src, EdgeWeightType& weight) const {
        ...
        if (data.activeIter == iter) {
            return std::make_pair(BFSUpdate<EdgeWeightType>(data.distance + weight, src->vid()), true);
        } else {
            return std::make_pair(BFSUpdate<EdgeWeightType>(), false);
        }
    }
    ...
    struct Task genScatterTask(const GraphGASLite::IterCount& iter, Ptr<VertexType>& src, EdgeWeightType& weight, const GraphGASLite::VertexIdx& dstId) const {
        auto& data = src->data();
        ReplaceParent<uint32_t, uint64_t>* awrp = new ReplaceParent<uint32_t, uint64_t>();
        ...
        struct Task task = {UINT_REPLACE_PARENT, (uint64_t)dstId, false, (void*)awrp, (uint64_t)(src->vid()), this->getVertexTid(dstId), this->getVertexTid(src->vid())};
    ...
    struct Task genScatterTask(Ptr<VertexType> src, const ShareVec& vertexData, EdgeWeightType weight, uint64_t srcId, uint64_t srcTid, uint64_t dstId, uint64_t dstTid, bool isDummy=false) const {
        ...
        if (vertexData.shares.size() != 2) {
            printf("Wrong vertex data shares length for BFS Scatter");
        }
        ...
        struct Task task = {UINT_REPLACE_PARENT, dstId, false, (void*)awrp, srcId, dstTid, srcTid};
    ...
    struct Task genDummyScatterTask(const Task& src_task) const {
        ReplaceParent<uint32_t, uint64_t>* awrp = new ReplaceParent<uint32_t, uint64_t>();
        ReplaceParent<uint32_t, uint64_t>* src_awrp = (ReplaceParent<uint32_t, uint64_t>*)src_task.buf;
        *awrp = *src_awrp;
    ...
    UpdateType getScatterTaskResult(const Task& task) const {
        ...
        ReplaceParent<uint32_t, uint64_t>* awrp = (ReplaceParent<uint32_t, uint64_t>*)task.buf;

        if (!awrp->isOperandEncrypted[0]) {
            // printf("Scatter result = %d, %lld\n", add->operands[0].plain, task.srcIndex);
            BFSUpdate<EdgeWeightType> result = BFSUpdate<EdgeWeightType>(awrp->operands[0].plain_val, awrp->operands[0].plain_parent);
            return result;
        } else {
            BFSUpdate<EdgeWeightType> result = BFSUpdate<EdgeWeightType>(awrp->operands[0].enc);
            return result;
        }        
    }
...
protected:
    BFSEdgeCentricAlgoKernel(const string& name, const GraphGASLite::VertexIdx& src)
...
#endif // ALGO_KERNELS_EDGE_CENTRIC_BFS_BFS_H_
