#ifndef ALGO_KERNELS_EDGE_CENTRIC_CC_CC_H_
#define ALGO_KERNELS_EDGE_CENTRIC_CC_CC_H_
...
//    };
    ...
    CCData(const GraphGASLite::VertexIdx&)
...
struct CCUpdate {
    ...
    CCUpdate(const EdgeWeightType distance_ = INF(EdgeWeightType), const GraphGASLite::VertexIdx& predecessor_ = INV_VID)
        : distance(distance_), predecessor(predecessor_), isEncrypted(false)
    {
        // Nothing else to do.
    }

    CCUpdate(const CipherEntry& c)
        : enc(c), isEncrypted(true)
    {
        // Nothing else to do.
    }

    CCUpdate& operator+=(const CCUpdate& update) {
        // Min of distance.
    ...
    static Task genTask(const CCUpdate& update1, const CCUpdate& update2, uint64_t vertexIndex, uint64_t srcTid, uint64_t dstTid, bool isFinal=false, bool isDummy=false) {
    ...
    // static Task genEmptyTask(const CCUpdate& update1, const CCUpdate& update2, uint64_t vertexIndex, uint64_t srcTid, uint64_t dstTid, bool isFinal=false) {
    //     MinWithParent<uint32_t, uint64_t>* mwp = new MinWithParent<uint32_t, uint64_t>();
    //     struct Task task = {MIN_UINT_WITH_PARENT, -1, false, (void*)mwp, -1, -1, -1, 0, isFinal};
    //     return task;
    // }

    static CCUpdate<EdgeWeightType> getTaskResult(const struct Task& task) {
        if (!task.finished) {
        ...
        if (!mwp->isOperandEncrypted[0]) {
            CCUpdate<EdgeWeightType> result = CCUpdate<EdgeWeightType>(mwp->operands[0].plain_val, GraphGASLite::VertexIdx(mwp->operands[0].plain_parent));
            return result;
        } else {
            CCUpdate<EdgeWeightType> result = CCUpdate<EdgeWeightType>(mwp->operands[0].enc);
    ...
    static CCUpdate<EdgeWeightType> splitRandomShareFromUpdate(const uint32_t srcTid, const uint32_t dstTid, CCUpdate<EdgeWeightType>& update) {
        // The update variable must be encrypted by the public key of dstTid
        CCUpdate<EdgeWeightType> update_share;
    ...
    static void mergeUpdateShare(const uint32_t dstTid, CCUpdate<EdgeWeightType>& update, CCUpdate<EdgeWeightType>& update_share) {
        CryptoUtil& cryptoUtil = CryptoUtil::getInstance();
...
void print(const CCUpdate<uint32_t>& update) {
    // printf("CCUpdate---------\n");
    // printf("count %d\n", update.count.cnt());
...
class CCEdgeCentricAlgoKernel : public GraphGASLite::SSEdgeCentricAlgoKernel<GraphTileType> {
public:
    static Ptr<CCEdgeCentricAlgoKernel> instanceNew(const string& name,
            const GraphGASLite::VertexIdx& src) {
        return Ptr<CCEdgeCentricAlgoKernel>(new CCEdgeCentricAlgoKernel(name, src));
    }
...
protected:
    ...
    std::pair<UpdateType, bool> scatter(const GraphGASLite::IterCount& iter, Ptr<VertexType>& src, EdgeWeightType& weight) const {
        ...
        if (data.activeIter == iter) {
            return std::make_pair(CCUpdate<EdgeWeightType>(data.distance + weight, src->vid()), true);
        } else {
            return std::make_pair(CCUpdate<EdgeWeightType>(), false);
        }
    }
    ...
    struct Task genScatterTask(Ptr<VertexType> src, const ShareVec& vertexData, EdgeWeightType weight, uint64_t srcId, uint64_t srcTid, uint64_t dstId, uint64_t dstTid, bool isDummy=false) const {
        ...
        if (vertexData.shares.size() != 2) {
            printf("Wrong vertex data shares length for CC Scatter");
        }
    ...
    UpdateType getScatterTaskResult(const Task& task) const {
        ...
        if (!awrp->isOperandEncrypted[0]) {
            ...
            CCUpdate<EdgeWeightType> result = CCUpdate<EdgeWeightType>(awrp->operands[0].plain_val, awrp->operands[0].plain_parent);
            return result;
        } else {
            CCUpdate<EdgeWeightType> result = CCUpdate<EdgeWeightType>(awrp->operands[0].enc);
            return result;
        }        
    }
...
protected:
    CCEdgeCentricAlgoKernel(const string& name, const GraphGASLite::VertexIdx& src)
...
#endif // ALGO_KERNELS_EDGE_CENTRIC_CC_CC_H_
